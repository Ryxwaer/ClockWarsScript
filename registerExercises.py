...
while ((datetime.now().minute * 60000000) + datetime.now().microsecond + datetime.now().second * 1000000) - start < 500000:
    try:
        counter += 1
        r = requests.get('https://xpolic05:ojtera6mon@wis.fit.vutbr.cz/FIT/st/course-sl.php.cs?xml=' + token + '&id=' + ident + '&log_85353=xml', headers=headers, timeout=requestTimeout)
